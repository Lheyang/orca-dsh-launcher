...
### Fixed
- **计价徽标按官方新规调整**：自 2026-08-23 起，周六/周日全天为低谷价，工作日峰谷时段（09:00-12:00、14:00-18:00 为高峰）与价格不变；提示卡新增周末说明，模型名对齐官方 `V4-Pro` / `V4-Flash`。

## [v2.1.0] - 2026-08-22

### Added
- **一键更新 DSH 支持预览更新内容**：点「更新 DSH…」时先做一次只读 `git fetch`，把即将拉取的提交（`HEAD..上游` 的 oneline）逐条列进确认框，用户看清「这次更新会带来什么」再决定是否继续；本地已是最新时直接提示、不再空跑 `git pull`，获取预览失败时降级提示但仍可继续。对应新增 `Orca.Core.DshServer.PreviewUpdate` 与 `UpdatePreview` 类型。

### Changed
- **视觉精修 + 呼吸感（控制台 / 安装向导）**：
  - 统一圆角（按钮与卡片 6→8）、所有内容卡片加细描边（主题自适应）；
  - 概览 / 安装状态卡改用「图标徽章」（圆角方块色块 + 强调色对勾），侧边栏当前页加强调色左侧指示条，状态徽章与标题栏加分隔/描边；
  - **呼吸感**：页面标题下方强调色短横线、主/次/危险按钮悬停「上浮」阴影、卡片悬停微高亮；
  - **浅色主题**：安装向导改为跟随用户主题（不再强制深色）、日志/安装日志等文本框改为主题自适应、目录选择框随主题变色，浅色下同样精致。
